...
def compute_ndcg_k(pred_items, test_items, test_indices, k, args):
...
def evaluate(u_emb, i_emb, Rtr, Rte, args, k = 10):
    ...
    for ue_f, tr_f, te_f in zip(ue_splits, tr_splits, te_splits):
        ...
        pred_items.scatter_(dim=1, index=test_indices, src=torch.ones_like(test_indices).float().to(args.device))
        ...
        ndcg = compute_ndcg_k(pred_items, test_items, test_indices, k, args)

        recall_k.append(rec)
        ndcg_k.append(ndcg)
    
    ndcg_final = torch.cat(ndcg_k).mean()
    recall_final = torch.cat(recall_k).mean()
    
    mlflow.log_metric("ndcg_k", ndcg_final)
    mlflow.log_metric("recall_k", recall_final)

    return ndcg_final, recall_final